...
public class SwingLableBarWeight extends HorizontalScrollView {
	...
	private int tabBackgroundResId = R.drawable.tab_slt_lable_bg;

	private Locale locale;

	public SwingLableBarWeight(Context context) {
		this(context, null);
	}

	public SwingLableBarWeight(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	@SuppressWarnings("ResourceType")
	public SwingLableBarWeight(Context context, AttributeSet attrs, int defStyle) {
		...
		a = context.obtainStyledAttributes(attrs,
				R.styleable.SwingLableBarWeight);

		indicatorColor = a.getColor(
				R.styleable.SwingLableBarWeight_pstsIndicatorColor,
				indicatorColor);
		underlineColor = a.getColor(
				R.styleable.SwingLableBarWeight_pstsUnderlineColor,
				underlineColor);
		dividerColor = a.getColor(
				R.styleable.SwingLableBarWeight_pstsDividerColor, dividerColor);
		indicatorHeight = a.getDimensionPixelSize(
				R.styleable.SwingLableBarWeight_pstsIndicatorHeight,
				indicatorHeight);
		underlineHeight = a.getDimensionPixelSize(
				R.styleable.SwingLableBarWeight_pstsUnderlineHeight,
				underlineHeight);
		dividerPadding = a.getDimensionPixelSize(
				R.styleable.SwingLableBarWeight_pstsDividerPadding,
				dividerPadding);
		tabPadding = a.getDimensionPixelSize(
				R.styleable.SwingLableBarWeight_pstsTabPaddingLeftRight,
				tabPadding);
		tabBackgroundResId = a.getResourceId(
				R.styleable.SwingLableBarWeight_pstsTabBackground,
				tabBackgroundResId);
		shouldExpand = a.getBoolean(
				R.styleable.SwingLableBarWeight_pstsShouldExpand, shouldExpand);
		scrollOffset = a.getDimensionPixelSize(
				R.styleable.SwingLableBarWeight_pstsScrollOffset, scrollOffset);
		textAllCaps = a.getBoolean(
				R.styleable.SwingLableBarWeight_pstsTextAllCaps, textAllCaps);
	...
	private void addTab(final int position, View tab) {
		...
		if (tab instanceof TextView) {
			if (position == 0) {
				((TextView) tab).setTextColor(getResources().getColor(
						R.color.tab_text_selected));// 默认文字颜色，我写的。
			} else {
				((TextView) tab).setTextColor(getResources().getColor(
						R.color.tab_text_normal));// 默认文字颜色，我写的。
			}
		}
		tabsContainer
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageSelected(int position) {
			...
			// if (view instanceof ImageButton) {// 如果是图片是Tab页，我写的。
	...
	private void setColor(int position) {
		...
		switch (position) {
		case 0:
			textView0.setTextColor(getResources().getColor(
