...
import com.tajiang.leifeng.view.pullrefresh.ILoadingLayout.State;
...
public class PullToRefreshListView extends PullToRefreshBase<ListView> implements OnScrollListener {
    ...
    private OnScrollNoteListener onScrollNoteListener;
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        if (mListView.getChildCount() > 1) {
            if (firstVisibleItem >= 1) {
                if (onScrollNoteListener != null) {
                    onScrollNoteListener.onScrollNote(DensityUtils.dp2px(getContext(), 140));
                }
            } else {
                if (firstView == null) {
                    firstView = mListView.getChildAt(0);
                }
                if (onScrollNoteListener != null) {
                    onScrollNoteListener.onScrollNote(-firstView.getTop());
                }
            }
        }
    ...
    protected LoadingLayout createHeaderLoadingLayout(Context context, AttributeSet attrs) {
        return new HeaderAnimLoadingLayout(context, attrs);
    }
    ...
    public interface OnScrollNoteListener {
        public void onScrollNote(int x);
    }

    public void setOnScrollNoteListener(OnScrollNoteListener onScrollNoteListener) {
        this.onScrollNoteListener = onScrollNoteListener;
    }
